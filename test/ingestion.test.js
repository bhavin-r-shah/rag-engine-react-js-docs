...
test('warns when a document has no searchable body content', () => {
  const parsed = parseDocument({
    sourcePath: 'index.md',
  ...
  const normalized = normalizeDocument(parsed);
  assert.deepEqual(normalized.warnings, ['Document contains no searchable body content.']);
  assert.equal(normalized.displayMarkdown, '');
});
